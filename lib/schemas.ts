...
export const mentorshipRequestSchema = z.object({
  ...
  requestType: z.enum(["mentorship", "referral_advice", "career_guidance"]).default("mentorship"),
...
export const onboardingSchema = z.object({
  ...
  department: z.string().min(2).max(120),
  graduationYear: z.coerce.number().int().min(1946).max(2100),
  currentCompany: z.string().max(120).optional(),
  currentTitle: z.string().max(120).optional(),
  industry: z.string().max(120).optional(),
  locationCity: z.string().min(2).max(120),
  ...
  openToMentorship: z.coerce.boolean().default(false),
  openToReferrals: z.coerce.boolean().default(false),
